...
<form action="Given.php" method="post" name="Myform" style="display:block; text-align:right; margin-right:140px; margin-top:20px;" onsubmit="return validate();">
...
{
$getdate=$_POST['date'];
$q="select computer.Serial_Number as CSN,computer.Name as CN,owner.Reg_Number as ORN,owner.First_Name as OFN,owner.gender as OG,owner.Last_Name as OLN,owner.Department as OD,owner.Phone as OP,district.District_Name as DDN,keeper.Id_number as KIN,keeper.First_Name as KFN,keeper.Last_Name as KLN,keeper.gender as KG,keeper.phone as KP,give_out.give_date as GGD from computer INNER JOIN owner INNER JOIN give_out INNER JOIN keeper INNER JOIN status INNER JOIN district ON computer.Serial_Number=give_out.computer_id AND owner.Reg_Number=computer.Owner_Reg_Number AND keeper.keeper_Id=give_out.keeper_id AND computer.mode=status.id and keeper.District_Id=district.District_Id where give_out.give_date like '%$getdate%' ORDER BY give_out.id desc";
...
<td align="left"><?php echo "$give_date"; ?></td></tr>
<?php
}
?>
</table>
