...
<table align="center" id="gutanga" cellpadding="10">
<tr><td>
<form action="give.php" method="post" id="computer" ><!-- onsubmit="return keepgivesearch();"> -->
<label>S N<sup><u>0</u></sup>,Code, R N<sup><u>0</u></sup>:</label></td><td><input type="text" name="snumber" id="keepgive" placeholder="S number or Reg Number" autofocus />
&nbsp;<input type="submit" value="Ni nde" name="nde" style="border-bottom-style:none;" />
<?php
...
if($count==0)
{
echo"<script> alert('Ino computer ntayirimo, Mubanze muyinjize muri system.');
window.location.herf('keep.php');</script>";
}
else
{
...
<?php
if($mode_Id==2)//activate
{
echo" <script> alert('Iyi computer Natyiri mu bubiko.');
...
<form action="give1.php" method="post">
<input type="text" name="snumber1" readonly="readonly" value="<?php echo "$Snumber"; ?>" /> &nbsp;
<input type="submit" name="keep" value="YITANGE" /></form>
<?php
}
